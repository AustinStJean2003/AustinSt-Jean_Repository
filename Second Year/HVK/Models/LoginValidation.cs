...
namespace HVK.Models
{
    ...
    public sealed class LoginValidation : ValidationAttribute
    {
        ...
        protected override ValidationResult IsValid(object value, ValidationContext validationContext)
        {
            ...
            {
                var checkPhone = DBContext.Customers.Where(c => c.Phone == (string)FirstFieldValue);

                if (checkPhone.Any() && checkPhone.First().Password == (string)PasswordValue)
                {
                    return ValidationResult.Success;
                }
                else
                if (!checkPhone.Any() && !(string.IsNullOrEmpty((string)value)))
                {
            ...
            return new ValidationResult("This Record was not found");
        }
